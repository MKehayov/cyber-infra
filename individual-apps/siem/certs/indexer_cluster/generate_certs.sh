...
cp root-ca.pem ../../base/graylog/certs
cp root-ca.pem ../../base/wazuh/certs/indexer_cluster
...
openssl req -days 3650 -new -key node-key.pem -out node.csr -subj "/C=US/L=California/O=Company/CN=*.svc.cluster.local"

echo "create: node.pem"

openssl x509 -req -days 3650 -in node.csr -CA root-ca.pem -CAkey root-ca-key.pem -CAcreateserial -sha256 -out node.pem -extfile ../../certs/indexer_cluster/v3.ext
...
cp filebeat* ../../base/wazuh/certs/indexer_cluster